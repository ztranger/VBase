...
- [ ] **Отложено как отдельный поэтапный трек (обоснование ниже).** Готовит почву уже сделанное:
  `Scene::build` разбит на мир + `createGpuResources`/`rebuildGraphics` (P1-07) — граница
  GPU-ресурсов и сессии уже намечена. Полная декомпозиция (`ClientSession`/`ClientWorld`/
  `WorldPresentation`/`SceneResources`) — 5 шагов по доку, каждый «без изменения поведения» и с
  зелёными server/desktop/selftest. Регрессии здесь — геймплейные/визуальные (prediction/
  reconciliation, VFX, build-режим, UI), их НЕ ловит headless-`--selftest`; нужен плейтест на
  десктопе И устройстве после каждого шага. Поэтому вынесено в отдельную сессию с плейтестом, а не
  делается вслепую в батче хардненинга (док прямо предостерегает: не начинать декомпозицию
  одновременно с другими фиксами).
