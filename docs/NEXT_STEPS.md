...
- **G1 — здания + экономика.** Генератор → капающий ресурс в хранилище (с потолком).
  Здания сперва из файла сцены. Ресурс на HUD/`GameUi`. Клиент начинает рендерить по типу.
- **G2 — спавнеры + враги.** Спавнер по правилам плодит врагов (тип `Enemy`, капсула в
  `CollisionWorld`), те бегут к ядру базы (движение-к-цели + скольжение по стенам). Без боя.
