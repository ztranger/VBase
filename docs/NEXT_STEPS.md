...
- **G3-B — размещение зданий героем** (следующее). Build-команда (клиент→сервер) за ресурс,
  превью-призрак, клик/тап ставит здание. **Модель размещения — сетка** (решено на входе в G3;
  привязка к клеткам, проверка занятости, футпринт-коллайдеры). Играбельная соло-оборона.
