...
- ✅ **G4 — кооп (общая база).** Инфраструктура была готова (сервер держит N клиентов, у
  ...
  - **Осталось для «полного» коопа** (по желанию): hp/смерть/респаун героя — сейчас враги бьют
    только ядро, героев игнорируют (можно стоять в толпе врагов без последствий). Это придаёт
    обороне ставки; сделать отдельным шагом (враг таргетит ближайшего героя/ядро, hero hp,
    «повержен» → респаун через задержку или подъём союзником).
