...
Осталось по мелочи на десктопе: подать ввод (мышь/клавиатуру) в ImGui — сейчас
панель инициализируется, но события в неё не идут (управление через WASD); по
желанию — своя `assets/textures/crate.png` для текстурированного куба.
