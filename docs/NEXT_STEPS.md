...
(нет стены/здания между ними). Всё серверно (`GameWorld::step`), клиент не тронут
...
- **Проверено:** self-тест `HeroAttack` (headless): melee выкашивает врагов у полосы (~2 выживает),
  маг снарядами (~5), а сплошная стена по линии видимости блокирует атаку (~26 копятся). Все 15
  self-тестов OK; десктоп/сервер собраны; `GameWorld/Character/CharacterRoster` под aarch64 чисто.
