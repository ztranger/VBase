...
- **Архитектура перед/вместе с G3 (часть 2):**
  - **#4 Ресурс per-team** (ещё не сделано). Сейчас `GameWorld::resource_` — единый пул
    (team 0). Кооп/PvP (ядро дизайна) требуют пул на команду — сделать до G4.
...
- **G4 — кооп** (общая база: команды/владение, общий ресурс — тут же #4 ресурс per-team).
