...
- **Архитектура перед/вместе с G3 (часть 2):**
  ...
  - **#5 Таблица типов на клиенте** (частично). Пока `Tower` просто добавлен в существующие
    `switch` (рендер-диспетч, `pickBounds`, поля мешей `Scene`). Полное сведёние в одну
    таблицу-дескриптор (тип→меш/материал/yOffset/pickRadius, визуалы в конфиг) — когда типов
    станет ещё больше.
