...
- **Сцены** (`assets/scenes/`): `arena_open`, `maze` (серпантин), `pillars` (решётка),
  ...
  DFS-лабиринты — единственный путь). Генераторы `gen_scenes.py` и превью `parse_preview.py`
  (топ-даун карты с зоной достижимости — гоняют тот же BFS) пока в скретчпаде;
  *TODO-решение: положить в `tools/` под git.*
