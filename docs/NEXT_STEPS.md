...
**Снаряд башни** — сделан (клиентская косметика, урон авторитетно-мгновенный): по каждой башне
локальный кулдаун (`RemoteEntity.fireTimer` = tower.rate), при нуле — болт в ближайшего врага в range;
`Scene::Projectile` летит из верхушки в центр цели, рисуется вытянутым кубом (`projMesh_`/`projMat_`)
вдоль скорости. Без изменения протокола.
