...
- **Десктоп-клиент** (`desktop/`): GLFW + desktop GL 3.3, переиспользует общее
  ядро. Пока рендерит кубами (полноценный рендер — см. роадмап). Подключается к
  серверу и грузит те же ассеты с диска. Сборка+подключение+загрузка проверены.
...
- **Рендер** (`Renderer` интерфейс, `Renderer.h`): `createMesh/createSkinnedMesh/
  createTexture/createMaterial/renderFrame/aspectRatio`. Реализация — `GlRenderer`
  (GLES3). `VulkanRenderer` — под тот же интерфейс, сейчас исключён из сборки.
...
Шейдеры — инлайн-строки в `GlRenderer.cpp` (`#version 300 es`). Vulkan-шейдеры
(`shaders/*.vert/.frag`, компиляция glslc→SPIR-V) сейчас не используются.
...
- Десктоп-клиент использует упрощённые версии (мягкая коррекция + сглаживание),
  Android/Scene — полные (реплей + буфер).

## 5. Карта файлов

`app/src/main/cpp/` (общее ядро + Android):
- Платформонезависимое: `Character.*`, `Scene.*`, `FollowCamera.h`, `Input.h`,
  `MathUtil.h`, `Mesh.*`, `Model.*`, `Texture.h`, `RenderFrame.h`, `Renderer.h`,
  `Net.*`, `Assets.*`, `AssetSource.h`, `FileAssetSource.h`, `Log.h` (переносимый).
- Android-специфичное: `main.cpp` (GameActivity, ввод, цикл, AndroidAssetSource),
  `GlRenderer.*` (GLES3+EGL), `Font.*`, `VulkanProbe.*`.
...
`desktop/`: `main.cpp`, `DesktopRenderer.*` (минимальный GL 3.3), `GlCore.h`
(свой загрузчик GL-функций вместо GLEW), `CMakeLists.txt`, `build.bat`
(переиспользует `Character/Mesh/Net/Assets/Model` + glfw + enet).
...
- **Скачивание с GitHub release assets** (GLEW zip, Fox.glb с raw) стабильно
  ...
  заменён своим мини-загрузчиком (`desktop/GlCore.h`).
...
1. **Десктопный Renderer-бэкенд** под интерфейс `Renderer` (desktop GL 3.3, порт
   шейдеров `300 es`→`330 core`, материалы/скиннинг/UBO/инстансинг/HUD). Тогда
   десктоп-клиент через общий `Scene` + `FileAssetSource` покажет ту же сцену/лису,
   что и телефон. Самый крупный шаг.
2. **Порт Vulkan-бэкенда** под контракт `RenderFrame` (+ depth, per-object матрицы).
   Файлы `VulkanRenderer.*` есть, сейчас исключены из сборки.
...
4. Загрузка текстур лисы на десктопе (уже готова через stb — нужна лишь при рендере).
