...
2. **Порт Vulkan-бэкенда** под `RenderFrame` — в работе, фазами, десктоп-first
   (флаг `--vk`; загрузчик `VkApi.*`, заголовки в `third_party/vulkan/`). Фаза 0
   (очистка экрана) готова и запущена; дальше — пайплайны/depth, материалы,
   инстансинг, скиннинг, ImGui, затем Android. Подробности — NEXT_STEPS §2.
