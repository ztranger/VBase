...
- **Свой аватар**: предсказание (локальный `simulate`) + **reconciliation с реплеем**
  ...
  (`Scene::applySnapshot`, буфер `pending_`). Сервер потребляет ввод **по одному за тик** из
  FIFO-очереди на героя (`GameWorld::HeroInputBuf`) и аккает seq **потреблённого**, а не
  последнего полученного — иначе пачка вводов за один `poll` коалесилась бы (см. §6). При
  бэклоге догоняет +1 ввод/тик, при пустой очереди удерживает последний (без повтора прыжка).
...
- **Здоровье соединения (пинг/обрыв/реконнект).** Пинги слать вручную НЕ нужно — ENet сам
  шлёт keep-alive (интервал 500 мс) и меряет RTT (`peer->roundTripTime`). `NetClient` отдаёт
  наружу `status()` (`NetStatus` Offline/Connecting/Connected/Lost) и `pingMs()` (RTT, мс).
  Порог таймаута ужат `enet_peer_timeout(peer, 0, 4000, 8000)` — обрыв ловится за ~4–8 с
  (дефолт ENet — до 5–30 с), но кратковременный лаг-спайк соединение не рвёт. Наш `disconnect()`
  рвёт peer через `_now` и обнуляет его ДО `poll`, поэтому событие `DISCONNECT` в `poll` = только
  НЕПРОШЕННЫЙ обрыв → `Lost`. Клиентский слой (`Scene`): при `Lost` чистит застывших чужих +
  неподтверждённые вводы и для join-сессии авто-переподключается к тому же IP раз в 2 с
  (`serverIp_`/`wantReconnect_`); host к 127.0.0.1 и сознательный `leaveGame` не реконнектят.
  Пинг/статус выводятся на экран рядом с FPS (HUD платформы) и в панелях `GameUi` (Home/Debug).
...
- **Ввод-коалесинг на сервере съедал прыжок и давал rubber-band.** `setHeroInput` раньше
  перезаписывал весь `hero->input`, а сервер за один `poll` осушает всю очередь ENet ДО
  `tick` → из пачки вводов за кадр (клиентский хитч шлёт несколько `simulate`/тик) выживал
  только последний: одноразовый `jump` терялся, а движение, напредсказанное клиентом по
  нескольким seq, сервер симулировал один раз, но аккал последний seq → клиент выкидывал
  `pending_` без реплея и откатывался. Решение: FIFO-очередь ввода на героя
  (`GameWorld::HeroInputBuf`), потребление **по одному за тик** (multi-call `simulate` — тем же
  паттерном, что и реплей на клиенте), `ackSeq` = seq ПОТРЕБЛЁННОГО. Пустая очередь → удержание
  последнего (`jump=false`), бэклог → догон +1/тик, жёсткий предел очереди — от флуда/долгой смерти.
...
4. **Неткод — доведение**: ✅ delta-сжатие снапшотов (сделано, см. §4);
   ✅ пинг/латенси на экране + детект обрыва + авто-реконнект (сделано, см. §4); осталось —
   lag compensation (когда появится боёвка/хиты — сейчас неприменима); вынести
   настройку сервера/адреса.
5. **Геймплей**: мир из многих сущностей (акторы в список; `Character` уже обобщён).
6. ✅ Своя `crate.png` для текстурированного куба — сделано (генератор `assets/textures/gen_crate.py`).
