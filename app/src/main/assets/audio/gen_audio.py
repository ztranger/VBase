...
пер-звук громкости). Музыка — спокойный ambient-пад + разреженная мелодия; пад СТРОГО
периодичен (частоты и LFO кратны 1/loopLen), ноты мелодии затухают внутри петли, поэтому
miniaudio-луп бесшовен (sample[last+1] == sample[0]).
...
# --- Музыка: спокойный ambient-пад + редкая мелодия, строго периодический (бесшовный луп) ---
# Раньше был чистый низкий дрон с суб-басом 55 Гц — на десктопных динамиках читался как «гул».
# Теперь: воздушный пад Am add9 БЕЗ суб-баса (вес смещён в середину/верх) + разреженная
# колокольчиковая мелодия сверху. Пад строго периодичен (частоты кратны 1/loop, LFO — целое
# число циклов). Ноты мелодии полностью затухают ВНУТРИ петли (у обоих краёв ~0), поэтому шов
# петли пересекает только периодический пад — луп остаётся бесшовным.
...
def gen_music():
    ...
    # Пад Am add9: опора A2 намеренно ТИХАЯ (раньше низ доминировал -> гул), тело — A3/C4/E4,
    # сверху G4/B4/E5 для воздуха.
    pad_notes = [(110.00, 0.16),                              # A2 — лёгкая опора, тихо
                 (220.00, 0.55), (261.63, 0.52), (329.63, 0.55),  # A3 / C4 / E4 — тело
                 (392.00, 0.30), (493.88, 0.34), (659.25, 0.16)]  # G4 / B4 / E5 — воздух
    pad = np.zeros(n)
    for j, (freq, amp) in enumerate(pad_notes):
        f = snap(freq)
        k = (j % 3) + 1               # LFO амплитуды: целое число циклов за петлю -> периодично
        lfo = 1.0 + 0.22 * np.sin(2 * np.pi * (k * f0) * t + j * 1.3)
        pad += amp * lfo * np.sin(2 * np.pi * f * t + j * 0.7)
    pad /= np.max(np.abs(pad))        # нормируем пад отдельно

    # Разреженная мелодия (A-минор пентатоника), плакированные «колокольчики». Каждая нота
    # затухает в 0 к своему концу (жёсткий tail), первая стартует с нулевой атаки -> mel[0]~0
    # и mel[-1]~0: шов не рвётся.
    mel = np.zeros(n)
    melody = [(0.0, 440.00, 1.8), (2.0, 523.25, 1.6), (4.0, 659.25, 2.0),
              (6.0, 587.33, 1.6), (8.0, 523.25, 1.8), (10.0, 440.00, 2.0),
              (12.0, 329.63, 1.6), (14.0, 392.00, 1.9)]
    for start, freq, dur in melody:
        i = int(start * SR)
        seg = min(int(dur * SR), n - i)
        if seg <= 0:
            continue
        tt = np.arange(seg) / SR
        env = (1.0 - np.exp(-tt / 0.035)) * np.exp(-tt / (dur * 0.30))  # мягкая атака, экспо-спад
        env *= np.clip((dur - tt) / (dur * 0.15), 0.0, 1.0)            # гарантированный 0 к концу
        v = np.sin(2 * np.pi * freq * tt) + 0.22 * np.sin(2 * np.pi * 2 * freq * tt)  # +2-я гарм.
        mel[i:i + seg] += env * v
    mel /= (np.max(np.abs(mel)) + 1e-9)

    # Микс: пад — тихий фон, мелодия — сверху. Общее очень медленное «дыхание» (2 цикла/петля).
    breath = 0.9 + 0.1 * np.sin(2 * np.pi * (2 * f0) * t)
    out = (0.5 * pad + 0.62 * mel) * breath

    # Записываем БЕЗ краевого фейда (это петля): нормировка вручную, тихо.
    x = out / np.max(np.abs(out)) * 0.32
