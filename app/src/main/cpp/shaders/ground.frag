...
// Vulkan Ground fragment shader: как lit.frag, но дальняя земля плавно тонируется в цвет горизонта
// (frame.uFogColor.rgb) по горизонтальной дистанции от камеры — прячет край карты без общего тумана.
...
void main() {
    ...
    float d = length(frame.uViewPos.xz - vWorldPos.xz);
    float f = smoothstep(kFadeStart, kFadeEnd, d);
    c = mix(c, frame.uFogColor.rgb, f);
    if (frame.uFogColor.w > 0.0) {  // опциональный объёмный туман
        float dd = length(frame.uViewPos.xyz - vWorldPos);
        c = mix(c, frame.uFogColor.rgb, clamp(1.0 - exp(-frame.uFogColor.w * dd), 0.0, 1.0));
    }
