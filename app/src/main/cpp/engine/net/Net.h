...
// а при >65535 изменившихся заголовок разошёлся бы с payload = порча состояния у клиента.
// Реальный геймплей держится много ниже; это ceiling, а не цель (снапшот такого размера ещё и
// фрагментируется — сплит снапшота отдельная задача).
constexpr uint32_t kMaxEntities = 1024;
static_assert(kMaxEntities <= 0xFFFFu, "kMaxEntities должен влезать в uint16 (счётчики снапшота)");

// Валиден ли предложенный клиентом ackTick как новая база дельт: только МОНОТОННЫЙ (строго
// больше принятого — клиент не может «разподтвердить») и НЕ из будущего (<= текущего тика).
// Отбивает амплификацию full-снапшотов: ackTick=0/старьё/будущее отклоняется, база не
// откатывается (легитимный клиент шлёт последний применённый тик — он и растёт, и в прошлом).
// В заголовке (не только в .cpp) — чтобы покрывалось самотестом.
inline bool ackIsValid(uint32_t proposed, uint32_t accepted, uint32_t serverTick) {
...
public:
    ...
    uint32_t debugFullSnapshots() const;   // всего разослано full-снапшотов (антиамплификация/метрики)
    uint32_t debugDeltaSnapshots() const;  // всего разослано delta-снапшотов
