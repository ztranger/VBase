...
// (ядра / здания). Хранит только поле расстояний; направление — градиент этого поля
// («вниз по склону» к ближайшей цели). Недостижимые клетки — reachable=false.
class FlowField {
public:
    ...
    // или путь недоступен. Считается как градиент поля расстояний по 4 соседям —
    // непрерывный вектор (сглаживает лесенку клеточного пути). Стена/за картой
    // читается как «чуть выше текущей», поэтому вектор отталкивается от препятствия.
    // Если текущая клетка занята — шаг на проходимую соседнюю с минимальной дистанцией.
    Vec3 direction(int cx, int cz) const;
    // BFS-дистанция до ближайшей цели (для отладочного хитмапа). -1 = недостижимо / вне карты.
    int distanceAt(int cx, int cz) const;
...
private:
    ...
    int minX_ = 0, minZ_ = 0, w_ = 0, h_ = 0;
    std::vector<uint16_t> dist_;  // BFS-дистанция до ближайшей цели; kUnreach = недостижимо
