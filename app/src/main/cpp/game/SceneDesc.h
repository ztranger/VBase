...
struct SceneDesc {
    ...
    // Цвет горизонта/фона (sRGB, как его видно). Фон-очистка = он же; дальняя земля
    // (материал с шейдером `ground`) плавно тонируется в него — край карты сливается с фоном.
    Vec3 horizonColor{0.07f, 0.07f, 0.12f};  // дефолт ~ прежний тёмный фон (сцены без `horizon`)
